...
- 新建任务页不是空白页，而是“任务启动台 + 对话发起窗”：首屏需要同时提供任务模板、聊天式输入区、候选识别反馈，以及进入正式分析前的流程预期
- 历史任务工作区与新建任务工作区分离：新建任务态不展示旧任务的协作过程和结果预览语义，提交成功后才切换到正式工作区
...
- [2026-05-17] “默认新建任务”还不够，首屏必须有明显的任务启动结构；若只给一个大输入框，用户仍会觉得页面是空白或未进入任务流
